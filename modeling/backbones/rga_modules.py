...
class RGA_Module(nn.Module):
	...
	def forward(self, x):
		...
		if self.use_channel:
			...
			# return out,w_spatial
			return out,W_ys
class Part_RGA_Module(nn.Module):
	def __init__(self, in_channel, in_spatial, use_spatial=True, use_channel=True, \
		cha_ratio=8, spa_ratio=8, down_ratio=8):
		super(Part_RGA_Module, self).__init__()

		self.in_channel = in_channel	#2048
		self.in_spatial = in_spatial    #6
		...
		self.inter_channel = in_channel // cha_ratio	#2048//8
		self.inter_spatial = in_spatial // spa_ratio	#6//1
		...
		# Networks for learning attention weights
		if self.use_spatial:
			num_channel_s = 1 + self.inter_spatial     # 1+6=7
			self.W_spatial = nn.Sequential(
				nn.Conv2d(in_channels=num_channel_s, out_channels=num_channel_s//1,
						kernel_size=1, stride=1, padding=0, bias=False),
				nn.BatchNorm2d(num_channel_s//1),
				nn.ReLU(),
				nn.Conv2d(in_channels=num_channel_s//1, out_channels=1,
						kernel_size=1, stride=1, padding=0, bias=False),
				nn.BatchNorm2d(1)
			)
		if self.use_channel:	
	...
	def forward(self, x):
		...
		if self.use_spatial:
			...
			Gs = torch.matmul(theta_xs, phi_xs)
			Gs_in = Gs.permute(0, 2, 1).view(b, h*w, h, w)
			Gs_out = Gs.view(b, h*w, h, w) 				#[b,6,6,1]
			Gs_joint = torch.cat((Gs_in, Gs_out), 1) #[b,12,6,1]
			Gs_joint = self.gg_spatial(Gs_joint)  #[b,6,6,1]
		
			g_xs = self.gx_spatial(x)	#[b,256,6,1]
			g_xs = torch.mean(g_xs, dim=1, keepdim=True)  #[b,1,6,1]
			ys = torch.cat((g_xs, Gs_joint), 1)		#[b,7,6,1]
		...
		if self.use_channel:
			# channel attention
			xc = x.view(b, c, -1).permute(0, 2, 1).unsqueeze(-1)  #[b,6,2048,1]
			theta_xc = self.theta_channel(xc).squeeze(-1).permute(0, 2, 1)   #[b,2048,6]
			phi_xc = self.phi_channel(xc).squeeze(-1)   #[b,6,2048]
			Gc = torch.matmul(theta_xc, phi_xc)  #[b,2048,2048]
			Gc_in = Gc.permute(0, 2, 1).unsqueeze(-1) #[b,2048,2048,1]
			Gc_out = Gc.unsqueeze(-1) #[b,2048,2048,1]
			Gc_joint = torch.cat((Gc_in, Gc_out), 1)	#[b,4096,2048,1]
			Gc_joint = self.gg_channel(Gc_joint)	#[b,256,2048,1]

			g_xc = self.gx_channel(xc)	#[b,6,2048,1]
			g_xc = torch.mean(g_xc, dim=1, keepdim=True) #[b,1,2048,1]
			yc = torch.cat((g_xc, Gc_joint), 1)
...
# input = Variable(torch.FloatTensor(4, 2048, 6, 1))
# rga= RGA_Module(2048, (6*1), use_spatial=True, use_channel=True,
# 								cha_ratio=8, spa_ratio=1, down_ratio=8)
# rga(input)